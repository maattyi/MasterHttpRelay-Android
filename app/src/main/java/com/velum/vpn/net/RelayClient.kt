...
class RelayClient(
    ...
    @Volatile private var consecutiveBatchFailures = 0

    private data class CoalesceEntry(val leaderDef: CompletableDeferred<Response>, val followers: MutableList<CompletableDeferred<Response>> = mutableListOf())
    ...
    private suspend fun coalescedSubmit(url: String, payload: JsonObject): Response {
        ...
        return try {
            ...
            entry.leaderDef.complete(r); r
        } catch (e: Throwable) {
            coalesce.remove(url)
            entry.followers.forEach { it.completeExceptionally(e) }
            entry.leaderDef.completeExceptionally(e); throw e
        }
    }
